...
public class CommentServiceImplTest {
    ...
    public void remove() throws Exception {
        ...
        BoardDto boardDto = new BoardDto("hello", "hello", "asdf"); //새로운 게시물 넣기
        assertTrue(boardDao.insert(boardDto) == 1);
        Integer bno = boardDao.selectAll().get(0).getBno();
        System.out.println("bno = " + bno);

        commentDao.deleteAll(bno);
        CommentDto commentDto = new CommentDto(bno,0,"hi","qwer");//새로운 댓글 넣기

        assertTrue(boardDao.select(bno).getComment_cnt() == 0);
        assertTrue(commentService.write(commentDto)==1);//댓글 쓰기
