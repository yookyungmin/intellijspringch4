...
public class BoardDaoImplTest {
    ...
    @Test
    public void searchSelectPageTest() throws Exception{  //검색기능
        boardDao.deleteAll();
        for(int i =0; i<=20; i++){
            BoardDto boardDto = new BoardDto("title"+i, "asdfasdfasdf","asdf"+i); //데이터 검즈알수있게
            boardDao.insert(boardDto);
        }
        SearchCondition sc = new SearchCondition(1, 10, "title2", "T"); // //title2%
        List<BoardDto> list = boardDao.searchSelectPage(sc);
        System.out.println("list="+list);
        assertTrue(list.size()==2); //1~20 title2
    ...
    public void searchResultCntTest() throws Exception{
        ...
        SearchCondition sc = new SearchCondition(1, 10, "title2","T"); // //title2%
        int cnt = boardDao.searchResultCnt(sc); //반환타입 List
        System.out.println("cnt = " + cnt);
        assertTrue(cnt==2);//1~20 title2

        sc = new SearchCondition(1, 10, "asdf2","W"); // //title2%
        cnt = boardDao.searchResultCnt(sc); //반환타입 List
        System.out.println("cnt = " + cnt);
        assertTrue(cnt==2);
    }
    ...
    public void insertTestData()throws  Exception{
        boardDao.deleteAll();
        for(int i = 1; i<=220; i++){
            BoardDto boardDto = new BoardDto("title"+i, "no content", "asdf");//게시물 하나 만들기
            boardDao.insert(boardDto);
